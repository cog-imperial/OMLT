...
from pyoml.opt.neuralnet import NeuralNetBlock, FullSpaceNet, ReducedSpaceNet
...
def test_two_node_full_space():
    ...
    m.neural_net_block.set_neural_net(FullSpaceNet(pyo.tanh))
    m.neural_net_block.set_weights(w,b,n_inputs,n_outputs,n_nodes)
    m.neural_net_block.build()
...
def test_two_node_reduced_space():
    ...
    m = pyo.ConcreteModel()
    m.neural_net_block = NeuralNetBlock()
    m.neural_net_block.set_neural_net(ReducedSpaceNet(pyo.tanh))
    m.neural_net_block.set_weights(w,b,n_inputs,n_outputs,n_nodes)
    m.neural_net_block.build()
