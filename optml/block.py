            ...
            for (i,var) in enumerate(inputs_list):
                if var.lb == None: #set lower bound to input_bounds value
                    ...
                    if var.lb < input_bounds[i][0]:
                        warnings.warning("Variable {} lower bound {} is less tight then network definition bound {}".format(var,var.lb,input_bounds[i][0]))
                if var.ub == None:
                    ...
                    if var.ub > input_bounds[i][1]:
                        warnings.warning("Variable {} upper bound {} is less tight then network definition bound {}".format(var,var.ub,input_bounds[i][1]))

    def _setup_scaled_inputs_outputs(self, *, scaling_object=None, input_bounds=None, use_scaling_expressions=False):
        if scaling_object == None:
            ...
            self.__scaled_inputs_list = scaling_object.get_scaled_input_expressions(self.inputs_list)
            self.__scaled_outputs_list = scaling_object.get_scaled_output_expressions(self.outputs_list)
            #Bounds only set on unscaled inputs
            self._setup_input_bounds(self.inputs_list,input_bounds)

        else:
            #create pyomo variables for scaled and unscaled terms, input bounds are also scaled
            self.scaled_inputs_set = pyo.Set(initialize=range(len(self.__inputs_list)), ordered=True)
            self.scaled_inputs = pyo.Var(self.scaled_inputs_set, initialize=0)
            
            self.scaled_outputs_set = pyo.Set(initialize=range(len(self.__outputs_list)), ordered=True)
            ...
            scaled_input_expressions = scaling_object.get_scaled_input_expressions(self.inputs_list)
            unscaled_output_expressions = scaling_object.get_unscaled_output_expressions(self.scaled_outputs_list)

            #scaled input constraints
            for i in range(len(self.scaled_inputs_set)):
                self.__scale_input_con[i] = self.scaled_inputs[i] == scaled_input_expressions[i]
...
class OptMLBlockData(_BaseInputOutputBlockData):
    ...
    def build_formulation(self, formulation, input_vars=None, output_vars=None):
        ...
        super(OptMLBlockData, self)._setup_scaled_inputs_outputs(scaling_object=formulation.scaling_object, 
