...
def reduced_space_dense_layer(net_block, net, layer_block, layer, activation):
    ...
    if (
        not hasattr(layer_block, "_format")
        or layer_block._format == DEFAULT_MODELING_LANGUAGE
    ):
        ...
        layer_block.z = var_factory.new_var(layer.output_indexes, lang=block_lang)
        layer_block.activation_constraint = constraint_factory.new_constraint(
            *layer.output_indexes, lang=block_lang
        )
        layer_block.zhat = var_factory.new_var(layer.output_indexes, lang=block_lang)
