...
def test_scaled_inputs_outputs():
    ...
    # Test with unscaled input bounds
    m = ConcreteModel()
    xoffset = {(0, i): float(i) for i in range(3)}
    xfactor = {(0, i): 0.5 * (i + 1) for i in range(3)}
    yoffset = {(1, i): -0.25 * i for i in range(2)}
    ...
    # Unscale input bounds
    for k in input_bounds.keys():
        lb, ub = input_bounds[k]
        input_bounds[k] = ((lb * xfactor[k]) + xoffset[k], (ub * xfactor[k]) + xoffset[k])
    ...
    _setup_scaled_inputs_outputs(m.b1, scaler=scaler, unscaled_input_bounds=scaled_input_bounds)
    m.obj = Objective(expr=1)
    m.b1.inputs.fix(2)
    m.b1.outputs.fix(1)
    status = SolverFactory('ipopt').solve(m)
    ...
    _setup_scaled_inputs_outputs(m.b1, scaler=scaler, unscaled_input_bounds=input_bounds)
    m.obj = Objective(expr=1)
    m.b1.inputs.fix(2)
    m.b1.outputs.fix(1)
    status = SolverFactory('ipopt').solve(m)
